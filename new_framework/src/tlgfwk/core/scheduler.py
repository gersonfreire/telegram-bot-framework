...
class JobScheduler:
    ...
    def add_job(
        ...
        **trigger_kwargs
        ...
        """
        try:
            # NOVO: Verificação explícita para trigger
            if trigger is None:
                self.logger.error(f"[add_job] Trigger type is None! job_id={job_id}, func={func}, name={name}, description={description}, user_id={user_id}, chat_id={chat_id}, kwargs={trigger_kwargs}\nStack trace:\n{traceback.format_stack()}")
                raise ValueError("[add_job] Trigger type cannot be None. Verifique a chamada de add_job e forneça um trigger válido ('date', 'interval', 'cron').")
