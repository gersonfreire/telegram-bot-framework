...
from tlgfwk.core import TelegramBotFramework, command, admin_required, owner_required
from tlgfwk.plugins.base import PluginBase
from tlgfwk.utils.crypto import generate_key, encrypt_data, decrypt_data
...
class DemoPlugin(PluginBase):
    ...
    async def plugin_info_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        info = (
            ...
            f"🔄 <b>Status:</b> {'✅ Ativo' if self.is_enabled() else '❌ Inativo'}\n"
...
class DemoBot(TelegramBotFramework):
    ...
    async def demo_stats_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        stats_msg = (
            ...
            f"• Ativos: {len([p for p in self.plugin_manager.plugins.values() if p.is_enabled()]) if self.plugin_manager else 0}\n\n"
            f"💾 <b>Sistema:</b>\n"
    ...
    async def _show_plugins_demo(self, query):
        ...
        active_plugins = len([p for p in self.plugin_manager.plugins.values() if p.is_enabled()]) if self.plugin_manager else 0
        ...
        message = (
            ...
            f"• /plugins - Listar plugins\n"
            f"• /plugin - Gerenciar plugin\n"
            f"• /plugin_demo - Demo do plugin\n"
            f"• /plugin_info - Info do plugin\n\n"
            f"💡 <b>Funcionalidades:</b>\n"
            f"• Carregamento dinâmico\n"
            f"• Comandos customizados\n"
            f"• Isolamento de código\n"
            f"• Hot-reload (futuro)"
        )
        await query.edit_message_text(message, parse_mode='HTML')
    ...
    async def _show_stats_demo(self, query):
        ...
        message = (
            ...
            f"📋 <b>Comandos:</b>\n"
            f"• /demo_stats - Estatísticas detalhadas\n"
            f"• /stats - Estatísticas do sistema\n\n"
            f"💡 <b>Funcionalidades:</b>\n"
            f"• Coleta automática\n"
